...
push:
	rm -Rf /tmp/gh-pages
	git clone git@github.com:simplepie/text-encoder.git --branch gh-pages --single-branch /tmp/gh-pages
	rm -Rf /tmp/gh-pages/*
...
.PHONY: push-travis
push-travis:
	git clone https://github.com/simplepie/text-encoder.git --branch gh-pages --single-branch /tmp/gh-pages
	rm -Rf /tmp/gh-pages/*
	cp -Rf ./docs/_build/* /tmp/gh-pages/
	cp -Rf ./docs/redirect.tmpl /tmp/gh-pages/index.html
	touch /tmp/gh-pages/.nojekyll
	...
	cd /tmp/gh-pages/ && \
		git add . && \
		git remote add upstream "https://$$GH_TOKEN@github.com/simplepie/text-encoder.git" && \
...
analyze: lint test
	...
	- bin/phpmetrics --config $$(pwd)/phpmetrics.yml --template-title="Text Encoder" --level=10 src/
